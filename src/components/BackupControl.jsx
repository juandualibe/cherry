// src/components/BackupControl.jsx
...
const BackupControl = () => {
  ...
  const diasHastaProximoBackup = () => {
    const ultimoBackup = localStorage.getItem('ultimoBackup');
    if (!ultimoBackup) return 5;
  ...
  if (minimizado) {
    return (
      <div 
        ...
        title="Abrir panel de backup"
      >
        💾 <span style={{fontSize: '0.9rem', fontWeight: '600'}}>{diasHastaProximoBackup()}d</span>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <p style={{fontSize: '0.85rem', margin: '0.5rem 0', color: '#666'}}>
        Último backup: <strong>{obtenerInfoBackup()}</strong>
      </p>
      <p style={{fontSize: '0.85rem', margin: '0.5rem 0', color: '#666'}}>
        Próximo backup: <strong>{diasHastaProximoBackup()} días</strong>
      </p>
      <button 
